            ...
            <ScrollView style={style.MainScrollView}>
                ...
                <View>
                    ...
                    <View style={style.officerandsitesection}>
                        ...
                        <View style={style.officerandsitesection}>
                            {logData && logData.data && logData.data.length > 0 && logData.data.map((entry) => (
                                <TouchableOpacity key={entry._id} style={style.licensnceplacenumbeerview} onPress={() => { setSelectedLog(entry); setIsModalVisibletwo(true); }}>
                                    ...
                                    <Text><Text style={style.onltfontweight}>Post/Shift: </Text>{entry.postShift}</Text>
                                    <Text><Text style={style.onltfontweight}>Note: </Text>{entry.note}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                    </View>
                        ...
                        <View style={style.logdatamodalview}>
                            {selectedLog ? (
                                <>
                                    ...
                                    <Text><Text style={style.onltfontweight}>Post/Shift: </Text>{selectedLog.postShift}</Text>
                                    <Text><Text style={style.onltfontweight}>Entered By: </Text>{selectedLog.enteredBy}</Text>
                                    <Text><Text style={style.onltfontweight}>Note: </Text>{selectedLog.note}</Text>
                                </>
                            ) : (
