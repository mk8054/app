...
const Policymanual = ({ navigation }) => {
    ...
    const passonlogdata = async () => {
        ...
        const apiUrl = `https://officer-reports-backend.onrender.com/api/policy-manual/getAll`;
            ...
            <ScrollView style={style.MainScrollView}>
                ...
                <View>
                    ...
                    <View style={style.officerandsitesection}>
                        ...
                        <View>
                            {data.map((item) => (
                                <TouchableOpacity key={item._id} style={style.hrcategorttouchopacity} onPress={() => { setSelectedLog(item); setIsModalVisible(true); }}>
                        ...
                        <View style={style.logdatamodalview}>
                            {selectedLog ? (
                                <>
                                    ...
                                    <Text><Text style={style.onltfontweight}>Category: </Text>{selectedLog.categoryName.categoryName}</Text>
                                    <Text><Text style={style.onltfontweight}>Subcategory: </Text>{selectedLog.subCategoryName.categoryName}</Text>
                                    <Text><Text style={style.onltfontweight}>Title: </Text>{selectedLog.title}</Text>
                                    <Text><Text style={style.onltfontweight}>Details: </Text>{selectedLog.details}</Text>
                                </>
                            ) : (
