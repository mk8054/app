  ...
  const fetchScheduleData = async option => {
    try {
      ...
      const response = await fetch(
        `https://officer-reports-backend.onrender.com/api/schedule/getByUserId/${userId}`,
        {
          method: 'GET',
          headers: {
          ...
          scheduleData.data.length > 0 && (
            <>
              ...
              <View style={style.dropdownList}>
                <TouchableOpacity
                  onPress={showDaySection}
                  style={[
                    style.optionButton,
                    selectedOption === 'Day' && style.selectedOption,
                  ]}>
                  <Text style={style.optionText}>Day</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={showWeeklySection}
                  style={[
                    style.optionButton,
                    selectedOption === 'Weekly' && style.selectedOption,
                  ]}>
                  <Text style={style.optionText}>Weekly</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={showMonthlySection}
                  style={[
                    style.optionButton,
                    selectedOption === 'Monthly' && style.selectedOption,
                  ]}>
              ...
              <View style={style.overallscheduleview}>
                <View>
                  <Text style={style.wednesdayanddate}>
                    {scheduleData.data[currentDateIndex]?.currentDate}
                  </Text>
                  <View style={style.totalworkinghoursmainview}>
                    <View style={style.overall24hoursmainview}>
                      ...
                      {scheduleData.data[currentDateIndex]?.dates
                        .slice(0, 1)
                        .map((date, idx) => (
                          <Text
                            key={idx}
                    ...
                    return (
                      <View key={idx} style={style.fullworkinghours}>
                        ...
                        <View
                          style={[
                            style.borderview,
                            {backgroundColor: isDutyHour ? '#ADD8E6' : 'white'},
                          ]}>
                          <Text />
                        </View>
                      </View>
                  ...
                  {scheduleData?.data?.map((week, index) => (
                    <View key={index}>
                      {week?.dates?.slice(0, 7).map((date, dateIndex) => (
                        <View
                          style={style.mainviewofdateofweekly}
                          key={dateIndex}>
                          <View style={style.viewofweeklysection}>
                            <View style={style.weeklysection}>
                              <Text style={style.showlargedateweekly}>
                                {formatDate(date?.date)?.day}
                              </Text>
                              <View>
                                <Text style={style.dayfoweeklydatesection}>
                                  {date?.day}
                                </Text>
                                <Text style={style.weeklydatesectiontext}>
                          ...
                          <View style={style.timesectionofweekly}>
                            <Text style={style.timetextsectionofweekly}>
                              {date?.startTime} - {date?.endTime}
                            </Text>
                          </View>
                          <View style={style.mainviewofeventsection}>
                            <View style={style.viewofsquareandrefreshicon}>
                              ...
                              />
                              <Text style={style.poganfunarmedtext}>
                                POG Unarmed
                              </Text>
                            </View>
                            ...
                            <Text style={style.officersinevent}>
                              Officers :
                            </Text>
                          </View>
                        </View>
