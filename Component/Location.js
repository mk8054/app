...
const LocationComponent = ({ navigation }) => {
    ...
    const InOutData = async () => {
        const token = await AsyncStorage.getItem('token');
        const clientSiteId = await AsyncStorage.getItem('siteid');
        const response = await fetch(`https://officer-reports-backend.onrender.com/api/clockInOut/get/${clientSiteId}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        ...
        <View style={style.Modalbackgroundmainview}>
            <View style={style.viewforwidthofclockinout}>
                ...
                <View style={style.clockpopupview}>
                    ...
                    <Text style={style.cloeckedoutbigtext}>{isShiftStarted ? "Clocked In" : "Clocked Out"}</Text>
                </View>
                <Text style={style.simpleborderline}></Text>
                    ...
                    {!isShiftStarted ? (
                        <TouchableOpacity onPress={lotiandlong}>
                            <View style={style.shiftstartimageview}>
                                <Image style={style.reportofofficerpasge} source={require('../assets/images/startshiftarrival.png')} />
                            </View>
                        </TouchableOpacity>
                    ) : (
                        <TouchableOpacity style={style.clockedbutnstyle} onPress={handleEndShift}>
