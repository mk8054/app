...
const ParkingViolationSearch = ({navigation}) => {
  ...
  const handlesubmit = async () => {
    try {
      ...
      const response = await fetch(
        'https://officer-reports-backend.onrender.com/api/parking-violation/search',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <View style={style.officerandsitesection}>
          ...
          <View style={style.textfieldandtextarseview}>
            ...
            <View style={style.maindropdownview}>
              <View style={style.dropdownmenuandiconview}>
                <Picker
                  selectedValue={licensePlateComparison}
                  onValueChange={itemValue =>
                    setlicensePlateComparison(itemValue)
                  }
                  style={style.dropdownheadingtext}>
          ...
          <View style={style.textfieldandtextarseview}>
            <View style={style.textandstariconview}>
              <Text style={style.onltfontweight} />
            </View>
            ...
            violationdata.map((violation, index) => (
              <View key={index} style={style.officerandsitesection}>
                ...
                <View style={style.licensnceplacenumbeerview}>
                  <Text>
                    <Text style={style.licenseplatenumbertext}>Date: </Text>
                    {new Date(violation.createdAt).toLocaleString()}
                  </Text>
                  <Text>
                    <Text style={style.licenseplatenumbertext}>LP: </Text>
                    {violation[0]?.data?.licensePlateNumber}
                  </Text>
                  <Text>
                    <Text style={style.licenseplatenumbertext}>VIN: </Text>
                    {violation.data.vin}
                  </Text>
                </View>
              </View>
