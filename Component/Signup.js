...
import { faUser, faEnvelope, faLock, faPhone, faEye } from '@fortawesome/free-solid-svg-icons';
...
const Signup = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        if (validateForm()) {
            try {
                const response = await fetch("https://officer-reports-backend.onrender.com/api/officer/create", {
                    method: 'POST',
                    headers: {
        ...
        <ScrollView style={style.MainScrollView}>
            <View style={style.officerimageviewinsignup}>
                <View style={{ width: 110, height: 80 }}>
                    <Image style={{ width: '100%', height: '100%' }} source={require('../assets/images/securitybaselogo.png')} />
                </View>
            </View>
            <View style={style.signuptextview}>
                <Text style={style.fewthingtostart}>Sign - up</Text>
                <Text style={{ textAlign: "center" }}>Just a few quick things to get started</Text>
            </View>
            ...
            {errors.userName && <Text style={style.signuperrortext}>{errors.userName}</Text>}
            <View style={style.signuptextandtextinputview}>
            ...
            {errors.emailAddress && <Text style={style.signuperrortext}>{errors.emailAddress}</Text>}
            <View style={style.signuptextandtextinputview}>
            ...
            {errors.phone && <Text style={style.signuperrortext}>{errors.phone}</Text>}
            <View style={style.signuptextandtextinputview}>
            ...
            {errors.password && <Text style={style.signuperrortext}>{errors.password}</Text>}
            <View style={style.signuptextandtextinputview}>
                <FontAwesomeIcon style={style.signupicon} icon={faLock} />
                <TextInput
                    style={style.signuptextinput}
            ...
            {errors.confirmPassword && <Text style={style.signuperrortext}>{errors.confirmPassword}</Text>}

            <TouchableOpacity style={style.submitbtnview} onPress={handleSubmit}>
                <Text style={style.submittextforlogin}>Create account</Text>
            </TouchableOpacity>
            <Text style={style.signtextforalredyaccount}>Already have an account ? <Text style={style.loginheretext} onPress={() => navigation.navigate("Login")}> login here</Text></Text>
        </ScrollView>
    );
};
