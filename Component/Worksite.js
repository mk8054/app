  ...
  const getallsites = async () => {
    try {
      ...
      const response = await fetch(
        `https://officer-reports-backend.onrender.com/api/officer/get/${officerid}`,
        {
          method: 'GET',
          headers: {
      ...
      <View style={style.Mainschedulemainview}>
        ...
        <View style={style.worksitemainview}>
          ...
          {sites.map((site, index) => (
            <TouchableOpacity
              key={site._id}
              style={style.styleforclient}
              onPress={() => handleSitePress(site)}>
              <Text style={style.textforsitesandclient}>
                Client: {site.clientId.companyName}
              </Text>
              <Text style={style.textforsitesandclient}>
                Site: {site.siteName}
              </Text>
