...
const Dailyreport = ({ navigation, parsedSiteData }) => {
    ...
    const [relievingOfficerFirstName, setRelievingOfficerFirstName] = useState('');
    ...
    const handleSubmit = async () => {
        try {
            ...
            const submitResponse = await fetch('https://officer-reports-backend.onrender.com/api/daily-activity-report/add', {
                method: 'POST',
                headers: {
    ...
    const handleGetObservationTypes = async () => {
        try {
            const response = await fetch(`https://officer-reports-backend.onrender.com/api/type/getAll?category=Observation`);
    ...
    const handleAddObservation = async () => {
        if (!addobservation.observationTypeId || addobservation.observationTypeId.trim() === '') {
            Alert.alert('Error', 'Please select a Type of Observation.');
            return;
        }
        const response = await fetch(`https://officer-reports-backend.onrender.com/api/observation/add`, {
            method: 'POST',
        ...
        if (response.ok) {
            const newObservation = {
                ...
                typeName: observationTypes.find(type => type._id === addobservation.observationTypeId).typeName,
    ...
    const handleGetAllObservations = async () => {
        try {
            ...
            const response = await fetch('https://officer-reports-backend.onrender.com/api/observation/getAll');
            const data = await response.json();

            if (data.success) {
                const observations = data.data;
                const filteredObservations = observations.filter(observation => observation.siteId === siteId);
    ...
    const handleUpdate = async (_id) => {
        setLoading(true);
        const response = await fetch(`https://officer-reports-backend.onrender.com/api/observation/update/${_id}`, {
            method: 'PUT',
            headers: {
    ...
    const deleteobservation = async (_id) => {
        const response = await fetch(`https://officer-reports-backend.onrender.com/api/observation/delete/${_id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            ...
            <ScrollView style={style.MainScrollView}>
                ...
                <View>
                    ...
                    <View style={style.ShiftStartNotesmainview}>
                        <View style={style.viewforbgcandtext}>
                            <Text style={style.headingtextfordailyactivey}>Shift Start Notes</Text>
                        </View>
                    ...
                    <View style={style.ShiftStartNotesmainview}>
                        <View style={style.viewforbgcandtext}>
                            ...
                            <Text style={style.headingtextfordailyactivey}>For every observation, click the "Add Observation" button below.</Text>
                        </View>
                            ...
                            <View key={index} style={style.observationtextandbuttonview}>
                                <View style={style.observationtextview}>
                                    <View>
                                        ...
                                        <Text>{new Date(obs.observationDateDisplay).toLocaleString()}</Text>
                                        <Text>{obs.comments}</Text>
                                    </View>
                                    <Text style={style.redborderindailyreport}></Text>
                                </View>
                                <View style={style.viewforeditbutandtext}>
                                    <TouchableOpacity style={style.removetextandiconview} onPress={() => deleteobservation(obs._id)}>
                                        <FontAwesomeIcon size={12} color='grey' icon={faTrash} />
                                        <Text style={style.removetextofdailyreport}>Remove</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={style.removetextandiconview} onPress={() => { setEditObservation({ comments: obs.comments, observationTypeId: obs.observationTypeId._id }); setSelectedObservation(obs); seteditmodal(true); }}>
                                        <FontAwesomeIcon size={12} color='grey' icon={faPenToSquare} />
                        ...
                        <View style={style.viewforredlineandaddbutton}>
                            <TouchableOpacity style={style.addobservationtouchopcay} onPress={() => setIsModalVisibletwo(true)}>
                    ...
                    <View style={style.videoaudiophotoviewmainview}>
                        <View style={style.viewforbgcandtext}>
                            <Text style={style.headingtextfordailyactivey}>Relieving Officer Information</Text>
                        </View>
                        <View style={style.textfieldandtextarseview}>
                            <View style={style.textandstariconview}>
                                <Text style={style.textinputtext}>Relieving Officer First Name :</Text>
                                <FontAwesomeIcon style={style.staricon} size={8} icon={faStarOfLife} />
                            </View>
                            <TextInput
                                style={style.textyinputforshift}
                        ...
                        <View style={style.textfieldandtextarseview}>
                            <View style={style.textandstariconview}>
                                <Text style={style.textinputtext}>Relieving Officer Last Name :</Text>
                                <FontAwesomeIcon style={style.staricon} size={8} icon={faStarOfLife} />
                            </View>
                            <TextInput
                                style={style.textyinputforshift}
                    ...
                    <View style={style.videoaudiophotoviewmainview}>
                        <View style={style.viewforbgcandtext}>
                            <Text style={style.headingtextfordailyactivey}>Additional Notes</Text>
                        </View>
                        <View style={style.textfieldandtextarseview}>
                            <View style={style.textandstariconview}>
                                <Text style={style.textinputtext}>Special Instructions:</Text>
                ...
                <View style={style.Modalbackgroundmainview}>
                    <View style={style.viewforaddobservationmodal}>
                        ...
                        <View style={style.textfieldandtextarseview}>
                            ...
                            <View style={style.textyinputforshift}>
                                {observationTypes && observationTypes.length > 0 ? (
                                    <RadioGroup size={10} radioButtons={radioButtons} onPress={handleSelection} selectedId={selectedId} />
                                ) : (
                        ...
                        <View style={style.textfieldandtextarseview}>
                            ...
                            <TextInput style={style.specialinstructiontextarea} value={addobservation.comments} onChangeText={(text) => setAddObservation({ ...addobservation, comments: text })} />
                        </View>
                        <View style={style.submitandclearbutnview}>
                            <TouchableOpacity style={style.submitbuttonview} onPress={handleAddObservation}>
                                <Text style={style.submitextindailyreport}>Submit</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={style.submitbuttonview} onPress={() => setIsModalVisibletwo(false)}>
                    ...
                    <View style={style.viewforaddobservationmodal}>
                        ...
                        <View style={style.textfieldandtextarseview}>
                            ...
                            <View style={style.textyinputforshift}>
                                {observationTypes && observationTypes.map((type) => (
                                    <TouchableOpacity key={type._id} onPress={() => handleSelection(type._id)}>
                                        <Text style={{ color: selectedId === type._id ? 'blue' : 'black' }}>
                                            {type.typeName}
                                        </Text>
                                    </TouchableOpacity>
                                ))}
                        ...
                        <View style={style.textfieldandtextarseview}>
                            ...
                            <TextInput
                                ...
                                onChangeText={(text) => setEditObservation((prev) => ({ ...prev, comments: text }))}
                            />
                        </View>
                        <View style={style.submitandclearbutnview}>
                            <TouchableOpacity style={style.submitbuttonview} onPress={() => handleUpdate(selectedObservation._id)}>
                                <Text style={style.submitextindailyreport}>Update</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={style.submitbuttonview} onPress={() => seteditmodal(false)}>
                                <Text style={style.submitextindailyreport}>Cancel</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
